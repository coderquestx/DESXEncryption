...
public class DESX {
    ...
    private int[] desCrypt(int[] block, int crypt_type) {
        ...
        int[] result = concatenate(R, L);
        result = permutate(fp, result);
        return result;
    }
    ...
    public byte[] encrypt(byte[] data) {
        int[] block = stringToBitList(data);
        int blockCount = (block.length + 63) / 64;
        int padding = blockCount * 64 - block.length;
        int[] paddedBlock = new int[block.length + padding];
        System.arraycopy(block, 0, paddedBlock, 0, block.length);
        for (int i = block.length; i < paddedBlock.length; i++) {
            paddedBlock[i] = 0;
        }
        byte[] result = new byte[paddedBlock.length / 8];
        for (int i = 0; i < blockCount; i++) {
            int[] encryptedBlock = desCrypt(Arrays.copyOfRange(paddedBlock, i * 64, (i + 1) * 64), ENCRYPT);
            System.arraycopy(bitListToString(encryptedBlock), 0, result, i * 8, 8);
        }
        return result;
    }
    ...
    public byte[] decrypt(byte[] data) {
        int[] block = stringToBitList(data);
        int blockCount = (block.length + 63) / 64;
        int[] paddedBlock = new int[block.length];
        System.arraycopy(block, 0, paddedBlock, 0, block.length);
        byte[] result = new byte[paddedBlock.length / 8];
        for (int i = 0; i < blockCount; i++) {
            int[] decryptedBlock = desCrypt(Arrays.copyOfRange(paddedBlock, i * 64, (i + 1) * 64), DECRYPT);
            System.arraycopy(bitListToString(decryptedBlock), 0, result, i * 8, 8);
        }
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        DESX desx = new DESX(key);

        byte[] plainText = "Message!".getBytes();
        byte[] encryptedText = desx.encrypt(plainText);
        System.out.println("Encrypted message: " + new String(encryptedText));

        byte[] decryptedText = desx.decrypt(encryptedText);
